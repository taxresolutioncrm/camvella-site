...
if (!homepage.includes('rel="icon" type="image/png" sizes="96x96" href="/favicon-96.png"')) throw new Error('Camvella favicon build gate: homepage is missing the stable Google-supported PNG favicon')
const manifest = await readFile(new URL('manifest.json', dist), 'utf8')
if (!manifest.includes('"src": "/favicon-96.png"')) throw new Error('Camvella favicon build gate: manifest is missing the stable Google-supported PNG favicon')
