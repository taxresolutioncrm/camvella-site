...
async function loadHome(){
  ...
  content.innerHTML='<div class="grid">'+
    ...
    '<section class="panel"><h2>Recent requests</h2><div class="list">'+((requests||[]).length?(requests||[]).map(r=>'<div class="item"><b>'+esc(r.request_type)+'</b><span>'+date(r.created_at)+' · '+esc(r.priority)+'</span>'+(r.details?'<span>'+esc(r.details)+'</span>':'')+'<div style="margin-top:7px"><span class="pill">'+esc(r.status)+'</span></div></div>').join(''):empty('No service requests yet.'))+'</div></section>';
}
...
async function loadDocuments(){
  ...
  document.getElementById('uploadForm').onsubmit=async e=>{
    ...
    const allowedTypes=new Set(['application/pdf','image/jpeg','image/png','image/heic','image/heif','application/msword','application/vnd.openxmlformats-officedocument.wordprocessingml.document','text/plain']);
    if(!allowedTypes.has(file.type)){box.innerHTML=status('That file type is not allowed.',true);return}
...
async function init(){
  ...
  if(!user){
    ...
    loginUrl.searchParams.set('return_to',new URL('./portal.html',location.href).pathname);
