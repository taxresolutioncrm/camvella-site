...
-- R9 portal user can read own client record.
select set_config('request.jwt.claims','{"sub":"PORTAL_USER","role":"authenticated"}',true);
do $$
declare n integer;
begin
  select count(*) into n from public.clients where id='PORTAL_CLIENT'::uuid;
  perform pg_temp.result('R9 portal reads own client','1 row',n||' rows',n=1);
end $$;

-- R10 portal user cannot read unrelated clients.
do $$
declare n integer;
begin
  select count(*) into n from public.clients where organization_id='ORG_A'::uuid and id<>'PORTAL_CLIENT'::uuid;
  perform pg_temp.result('R10 portal isolated from other clients','0 rows',n||' rows',n=0);
end $$;

-- R11 cross-organization remains blocked for every role.
select set_config('request.jwt.claims','{"sub":"ADMIN_USER","role":"authenticated"}',true);
do $$
declare n integer;
begin
  select count(*) into n from public.clients where organization_id='ORG_B'::uuid;
  perform pg_temp.result('R11 admin cross-org isolation','0 rows',n||' rows',n=0);
end $$;
