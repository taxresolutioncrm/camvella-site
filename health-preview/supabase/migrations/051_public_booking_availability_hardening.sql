-- 051_public_booking_availability_hardening.sql
-- Returns booking metadata and prevents public callers from booking outside configured availability.
...
as $$
  ...
  order by s.starts_at,s.timezone;
$$;
...
begin
  ...
  -- The requested start must be one of the 15-minute-grid slots inside an
  -- active availability rule in that rule's timezone.
  if not exists (
    select 1
    from public.scheduling_availability_rules r
    where r.user_id=link_row.user_id
      and r.organization_id=link_row.organization_id
      and r.is_active=true
      and r.day_of_week=extract(dow from (p_starts_at at time zone r.timezone))::integer
