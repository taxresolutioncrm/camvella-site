-- 082_storage_path_authorization_alignment.sql
-- Storage authorization now validates organization/office/entity path segments before role checks.
...
declare
  org_id uuid;
  office_segment text;
  import_type_segment text;
  office_ok boolean:=false;
begin
  begin
    org_id:=split_part(p_name,'/',1)::uuid;
    office_segment:=split_part(p_name,'/',2);
    import_type_segment:=split_part(p_name,'/',3);
  exception when others then
    return false;
  end;

  if import_type_segment not in ('leads','clients','carrier_products') then
    return false;
  end if;

  if office_segment='shared' then
    office_ok:=true;
  else
    begin
      office_ok:=exists(
        ...
        where o.id=office_segment::uuid
