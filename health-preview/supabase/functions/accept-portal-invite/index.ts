...
export default {
  fetch: withSupabase({ auth:'user', cors:appCorsConfig(), errors:{detailed:false} }, async(req,ctx)=>{
    ...
    try{
      ...
      const {data,error}=await ctx.supabaseAdmin.rpc('accept_portal_invitation',{
        p_user_id:userIdFromClaims(ctx.userClaims as Record<string,unknown>),
        p_email:email,
        p_token:token
      });
      if(error) return response({error:'portal_invite_accept_failed',message:error.message},400);
