...
export default {
  fetch: withSupabase({ auth:'user', cors:appCorsConfig(), errors:{detailed:false} }, async(req,ctx)=>{
    ...
    try{
      ...
      const {data:membership,error:membershipError}=await ctx.supabase.from('memberships')
      ...
      if(membership.role==='agent'){
        const {data:client,error:clientError}=await ctx.supabase.from('clients')
          ...
          .eq('id',doc.client_id)
          .maybeSingle();
        if(clientError||!client||client.assigned_user_id!==userId) return response({error:'document_share_not_authorized'},403);
      }else if(!['agency_admin','manager','compliance'].includes(membership.role)){
        return response({error:'document_share_not_authorized'},403);
      }
