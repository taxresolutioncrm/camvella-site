...
export class ActionService{
 constructor(backend){this.backend=backend;this.repo=backend.repository;this.client=backend.client;this.workspace=backend.workspace;this.selected=backend.selected;this.orgId=backend.selected?.organization_id||null;this.officeId=backend.selected?.office_id||null;this.userId=backend.workspace?.user?.id||null}
 ...
 async execute(action,fields){
  ...
  if(a.includes('New ACA Case')){const c=await this.resolveClient(fields['Client']);const hh=Number(fields['Household size']);return this.repo.call('create_enrollment_case',{p_client_id:c.id,p_assigned_user_id:await this.resolveUser(fields['Agent']),p_market:'aca',p_enrollment_type:text(fields['SEP / Enrollment reason'])||null,p_household_size:Number.isFinite(hh)&&hh>0?Math.floor(hh):null})}
  if(a.includes('New Medicare Case')){const c=await this.resolveClient(fields['Client']);return this.repo.call('create_enrollment_case',{p_client_id:c.id,p_assigned_user_id:await this.resolveUser(fields['Agent']),p_market:'medicare',p_enrollment_type:[text(fields['Election period']),text(fields['Product focus'])].filter(Boolean).join(' · '),p_household_size:null})}
  if(a.includes('New Service Request')){const c=await this.resolveClient(fields['Client']);return this.repo.create('serviceRequests',{office_id:c.office_id||this.officeId,client_id:c.id,request_type:text(fields['Request type']),source:'agent',priority:priority(fields['Priority']),status:'open',assigned_user_id:this.userId})}
  if(a.includes('New Task'))return this.repo.create('tasks',{office_id:this.officeId,assigned_user_id:await this.resolveUser(fields['Assigned to']),title:text(fields['Task']),priority:priority(fields['Priority']),status:'open',due_at:fields['Due']?new Date(fields['Due']).toISOString():null,created_by:this.userId});
